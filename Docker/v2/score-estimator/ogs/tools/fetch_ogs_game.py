...
def fetch_game(game_id):
    res = requests.get(
        'https://online-go.com/api/v1/games/%d/' % game_id
    )
    ...
    clockdata = gamedata['clock']
    print("clockdata:",clockdata)

    #board = data['board']
    board = gamedata['moves']
    print("board:",board)
    board = [
                [-1 if x == 2 else x for x in row]
                for row in board
            ]
    #last_move = clockdata['last_move']
    last_move = board[-1]
    print("last_move:",last_move)
    player_to_move = 0
    if last_move[1] == -1:
        player_to_move = -board[last_move[1]][last_move[0]]
    if player_to_move == 0:
        player_to_move = 1
